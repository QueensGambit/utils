...
def chunk(n, nchunks):
    ...
    d = n / nchunks
    r = n - nchunks * d
    indices = list(range(0, (d + 1) * r, d + 1)) + list(range((d + 1) * r, n + d, d))
    indices[-1] = None
    a, b = tee(indices)
    next(b, None)
    return zip(a, b)
...
def sp_create_data(data, rows, cols, dim1, dim2, format):
    ...
    if format == "dok":
        ...
        return dict([i for i in zip(rows, data) if i[1] != 0])
    else:
...
def timerflag(func):
    def wrapper(*args, **kwargs):
        if 'timer' in kwargs and kwargs['timer']:
            del kwargs['timer']
            starttime = time.time()
